 ...
 * @package Risecommerce\GeoIp\Model
 */
class IpToCountryRepository
{
    /**
    ...
    public function getCountryCode($ip)
    {
        if (!isset($this->ipToCountry[$ip])) {
            ...
            if (!$this->ipToCountry[$ip]) {
                try {
                    $filename = $this->directoryList->getPath('var') . DIRECTORY_SEPARATOR . 'risecommerce/geoip/GeoLite2-Country.mmdb';
                    if (file_exists($filename)) {
                        $datFile = $filename;
                    } else {
                        $datFile = $this->moduleDir->getDir('Risecommerce_GeoIp') . '/data/GeoLite2-Country.mmdb';
                    }
